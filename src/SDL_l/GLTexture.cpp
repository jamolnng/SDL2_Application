...
bool GLTexture::create(unsigned int width, unsigned int height, float r, float g, float b, float a, int MIN_FILTER, int MAG_FILTER)
{
	...
	SDL_FillRect(surface, NULL, SDL_MapRGBA(surface->format, (Uint8)std::round(r * 255.0f), (Uint8)std::round(g * 255.0f), (Uint8)std::round(b * 255.0f), (Uint8)std::round(a * 255.0f)));
	return load(surface, MIN_FILTER, MAG_FILTER);
}
...
{
	if (value == 0 || pixels == NULL || sizeof(pixels) != sizeof(getPixels())) return false;
	glBindTexture(GL_TEXTURE_2D, value);
